...
const DeleteData = ({ Jabatan }) => {
    // console.log(user)
    const [Open, setIsOpen] = useState(false);
    const router = useRouter();

    const handleDelete = async () => {
        console.log('hapus', Jabatan.id)
        const result = await fetch(`/api/jabatan/${Jabatan.id}`, { method: "DELETE" })
        router.refresh();
        setIsOpen(false);
    };
                ...
                <div className="bg-black/50 fixed inset-0">
                    <div className="flex justify-center items-center h-full">
                        <div className="rounded-lg flex flex-col bg-slate-300 w-1/3 p-5">
                            ...
                            <h3 className="font-semibold text-center text-lg text-black">Anda ingin menghapus jabatan {Jabatan.nama} ?</h3>
