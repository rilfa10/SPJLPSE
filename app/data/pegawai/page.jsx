import React from 'react'
import AddData from './addPegawai';
import DeleteData from './deletePegawai';
import EditData from './editPegawai';
import { PrismaClient } from '@prisma/client';
import DetailData from './detail';
const prisma = new PrismaClient();

async function getDatapegawai() {
    return await prisma.pegawai.findMany({
        include: {
            jabatan: true,

        }
    })

}

async function getDatajabatan() {
    return await prisma.jabatan.findMany()
}

const DataPage = async () => {
    const datapegawai = await getDatapegawai()
    const datajabatan = await getDatajabatan()
    // console.log(datapegawai);
    return (
        <div className='bg-white rounded-lg mx-4 p-4'>
            <div className='grid'>
                <div className="col-12">
                    <div className='rounded-lg bg-slate-100 w-85 ml-2 mb-2 mr-2 border border-slate-300'><AddData jabatan={datajabatan} /></div>
                </div>
                <div className='col-12'>
                    <div className='bg-slate-100 w-85 ml-2 mr-2 p-2 border border-slate-300'>
                        <div className='flex justify-between m-2'>
                            <div className='text-xl items-center'> Tabel Pegawai</div>
                        </div>
                    </div>
                    <div className='rounded-b bg-slate-100 ml-2 mr-2'>
                        <div className='mb-2'>
                            {/* <addData /> */}
                        </div>
                        <table className='w-full'>
                            <thead className='rounded-lg bg-grey-50 p-2 border border-slate-300 divide-slate-300 '>
                                <tr>
                                    <th className='p-3 text-sm font-semibold tracking-wide text-center'>No</th>
                                    <th className='p-3 text-sm font-semibold tracking-wide text-center'>Nip</th>
                                    <th className='p-3 text-sm font-semibold tracking-wide text-center'>Nama</th>
                                    <th className='w-24 p-3 text-sm font-semibold tracking-wide text-center'>Action</th>
                                </tr>
                            </thead>
                            <tbody className='divide-y divide-grey-300'>
                                {datapegawai.map((Pegawai, index) => (
                                    <tr key={Pegawai.id}>
                                        <td className='p-3 text-sm text-gray-700 text-center whitespace-nowrape'>
                                            {index + 1}
                                        </td>
                                        <td className='p-3 text-sm text-gray-700 text-center whitespace-nowrape'>
                                            {Pegawai.nip}
                                        </td>
                                        <td className='p-3 text-sm text-gray-700 text-center whitespace-nowrape'>
                                            {Pegawai.nama}
                                        </td>
                                        {/* <td className='p-3 text-sm text-gray-700 text-center whitespace-nowrape'>
                                            {Pegawai.alamat}
                                        </td>
                                        <td className='p-3 text-sm text-gray-700 text-center whitespace-nowrape'>
                                            {Pegawai.telp}
                                        </td>
                                        <td className='p-3 text-sm text-gray-700 text-center whitespace-nowrape'>
                                            {Pegawai.jabatan.nama}
                                        </td> */}
                                        <td className='text-sm text-gray-700 whitespace-nowrape'>
                                            <div className='flex justify-center'>
                                                <DetailData Pegawai={Pegawai} />
                                                <EditData Pegawai={Pegawai} jabatan={datajabatan} />
                                                <DeleteData Pegawai={Pegawai} />

                                            </div>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                    </div>

                </div>
            </div>
        </div >
    )
}

export default DataPage;