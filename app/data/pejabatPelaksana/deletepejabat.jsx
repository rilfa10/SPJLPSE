...
const DeleteData = ({pejabatPelaksana }) => {
    // console.log(user)
    const [Open, setIsOpen] = useState(false);
    const router = useRouter();

    const handleDelete = async () => {
        console.log('hapus', pejabatPelaksana.id)
        const result = await fetch(`/api/pejabatPelaksana/${pejabatPelaksana.id}`, { method: "DELETE" })
        setIsOpen(false);
        router.refresh();
    };
                ...
                <div className="bg-black/50 fixed inset-0">
                    <div className="flex justify-center items-center h-full">
                        <div className="rounded-lg flex flex-col bg-slate-300 w-1/3 p-5">
                            ...
                            <h3 className="font-semibold text-lg text-center text-black">Anda ingin menghapus data {pejabatPelaksana.pegawai.nama} ?</h3>
